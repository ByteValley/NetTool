 ...
 * 作者：ByteValley
 * 版本：2025-11-27R3-FULL (budget-stable full)
 *
 * 功能边界
 ...
 *
 * 运行环境 · 依赖接口
 *  · Surge / Loon / Egern：$httpClient / $httpAPI（入口策略可用）/ $persistentStore|$prefs / $notification / $network
 *  · QuanX：$task.fetch（HTTP）/ $prefs / $notification / $network（入口策略不可用则自动降级）
 *
 * 预算稳态
 *  · BUDGET=0 自动：min(10s, Timeout) 内自适应，临近截止会压短单次请求 cap 并提前收工输出
 *  · 强兜底：到预算上限会自动 done()，避免“脚本超时”
 * ========================================================= */
...
/**
 * ENV：统一参数优先级
 *  · arguments 显式修改优先于 BoxJS；否则 BoxJS 覆盖默认；否则取 arguments/默认
 */
...
function budgetLow(ms) {
  return budgetLeft() < (Number(ms) || 0);
}

/**
 * 预算感知 cap：
 *  - 临近截止会把单次请求 cap 压短，避免把脚本拖到超时
 */
...
let __hardTimer = null;

function done(payload) {
  if (__doneOnce) return;
  __doneOnce = true;
  try { if (__hardTimer) clearTimeout(__hardTimer); } catch (_) {}
  $done(payload);
}

__hardTimer = setTimeout(() => {
  const title = maybeTify(netTypeLine() || t("unknownNet"));
  const content = maybeTify([
    `${t("runAt")}: ${now()}`,
    `${t("policy")}: -`,
    "",
    "⚠️ Budget reached，已提前结束以避免脚本超时"
  ].join("\n"));
  done({ title, content, icon: ICON_NAME, "icon-color": ICON_COLOR });
}, Math.max(1200, BUDGET_MS + 50));
...
const HTTP_ADAPTER = (() => {
  ...
  if (typeof $task !== "undefined" && $task.fetch) {
    return {
      kind: "taskFetch",
      get: (req) => $task.fetch({ url: req.url, method: "GET", headers: req.headers || {}, body: req.body }).then((r) => ({
        resp: { status: r.statusCode || r.status || 0, headers: r.headers || {} },
        body: r.body
      })),
      post: (req) => $task.fetch({ url: req.url, method: "POST", headers: req.headers || {}, body: req.body || "" }).then((r) => ({
        resp: { status: r.statusCode || r.status || 0, headers: r.headers || {} },
        body: r.body
      }))
    };
...
/** 面板关键链路 cap：随 SD_TIMEOUT 压短，避免“累计回退”拖死 */
const HTTP_CAP_MS = Math.min(3000, Math.max(1600, SD_TIMEOUT_MS));

function httpCall(method, req, timeoutMs = null, capMs = null, logTag = "HTTP") {
  return new Promise((resolve, reject) => {
    if (HTTP_ADAPTER.kind === "none") return reject(new Error("no-http-adapter"));
    ...
    const cap = capMs == null ? HTTP_CAP_MS : Number(capMs);
    const capped = capByBudget(Number.isFinite(cap) ? cap : HTTP_CAP_MS);
    ...
    const fn = (String(method).toUpperCase() === "POST") ? HTTP_ADAPTER.post : HTTP_ADAPTER.get;

    const run = async () => {
      // QuanX fetch 无原生 timeout：用 race 限制
      if (HTTP_ADAPTER.kind === "taskFetch") {
        return await withTimeout(fn(payload), to, null);
      }
      // httpClient：传 timeout 给内核（Surge/Loon/Egern）
      return await fn(Object.assign({}, payload, { timeout: to }));
    };
    ...
    Promise.resolve(run())
      .then((ret) => {
        ...
        if (!ret) {
          const cost = Date.now() - start;
          log("warn", `${logTag} fail`, req.url, "cost", cost + "ms", "timeout");
        ...
        resolve({ status, headers: resp.headers || {}, body: ret.body, cost });
      })
      .catch((err) => {
        if (doneFlag) return;
        doneFlag = true;
        clearTimeout(wd);
        const cost = Date.now() - start;
        log("warn", `${logTag} fail`, req.url, "cost", cost + "ms", String(err || "err"));
...
function httpGet(url, headers = {}, timeoutMs = null, followRedirect = false) {
  ...
  return httpCall("GET", req, timeoutMs, HTTP_CAP_MS, "HTTP GET");
}

function httpPost(url, headers = {}, body = "", timeoutMs = null) {
  const req = { url, headers, body };
  return httpCall("POST", req, timeoutMs, HTTP_CAP_MS, "HTTP POST");
}
...
// ====================== 统一抓取器（减少重复 / 回退限次） ======================
async function trySources(order, sourceMap, {
  preferLogTag,
  ...
  const list = Array.isArray(order)
    ? order.slice(0, maxTries ? Math.max(1, Math.min(Number(maxTries), order.length)) : order.length)
    : [];

  for (const key of list) {
    if (budgetLow(minLeftMs)) {
      log("warn", `${preferLogTag} budget break`, { left_ms: budgetLeft() });
      break;
    }
...
async function tryIPv6Ip(order, opt = {}) {
  ...
  const minLeftMs = Number(opt.minLeftMs || 900);

  for (const key of order.slice(0, maxTries)) {
    if (budgetLow(minLeftMs)) {
      log("warn", "IPv6 budget break", { left_ms: budgetLeft() });
      break;
    }
...
async function getDirectV6(preferKey) {
  ...
  const res = await tryIPv6Ip(order, { timeoutMs: V6_TO, maxTries: Math.min(2, order.length), minLeftMs: 900 });
...
// ====================== 落地区号查询（单次缓存，避免重复外联） ======================
let __sdCCOnce = null;
function sd_queryLandingCCMultiOnce() {
  if (!__sdCCOnce) __sdCCOnce = sd_queryLandingCCMulti();
  return __sdCCOnce;
}
...
async function sd_testChatGPTAppAPI() {
  ...
  if (!cc) cc = await sd_queryLandingCCMultiOnce();
...
async function sd_testDisney() {
  ...
  if (!rBam.ok || rBam.status !== 200) {
    const cc = homeCC || (await sd_queryLandingCCMultiOnce()) || "";
  ...
  const cc = blocked ? "" : ((bamCC || homeCC || (await sd_queryLandingCCMultiOnce()) || "").toUpperCase());
...
/**
 * 服务检测：整体限时 + 并发队列 + 预算自适应缩服务
 */
async function runServiceChecks() {
  let order = selectServices();
  if (!order.length) return [];

  // 预算不足：直接跳过，保证主面板输出
  if (budgetLow(1800)) {
    log("warn", "Budget low, skip service checks", { left_ms: budgetLeft() });
    return [];
  }

  // 接近截止：自动缩短服务数量
  const left = budgetLeft();
  if (left < 3500) order = order.slice(0, Math.min(2, order.length));
  else if (left < 5500) order = order.slice(0, Math.min(4, order.length));
  ...
  await withTimeout(
    new Promise((r) => {
      const tick = () => {
        ...
        if (budgetLow(260)) return r(true);
        setTimeout(tick, 30);
      };
      tick();
    }),
