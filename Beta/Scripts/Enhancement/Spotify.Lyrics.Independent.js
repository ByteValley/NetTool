...
function isMetadataTrackUrl(url){return /\/metadata\/\d+\/track\//.test(String(url||''))}
...
export default async function(ctx){
 try{
  if(!ctx.response){
   ...
   if(isContextTrackUrl(url))return prepareContextRequest(ctx.request,console.log);
   if(isMetadataTrackUrl(url))return prepareMetadataRequest(ctx.request,console.log);
   if(isExtendedMetadataUrl(url))return prepareUncompressedRequest(ctx.request,console.log,'extended-metadata 请求');
   if(isColorLyricsUrl(url))return prepareUncompressedRequest(ctx.request,console.log,'color-lyrics 请求');
   return;
  }
