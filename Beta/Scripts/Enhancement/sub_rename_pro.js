...
- 
- 修复说明（R2）：
- · Surge 使用 JavaScriptCore 引擎，const/let 声明的函数存在 TDZ 问题
- · 全部改为 function 声明（自动提升，无 TDZ），兼容 JavaScriptCore
- · var 替代 const/let 用于普通变量声明
- · 展开 for-of / forEach 为普通 for 循环
- · 展开 Object.entries 为 Object.keys 遍历
- ========================================================= */
...
s = s.replace(
new RegExp(”(” + ezh + “)\s+(” + ezh + “)(\s+(?:” + keepAlt + “)\b.*)?”, “gi”),
“$1$3”
);
s = s.replace(new RegExp(”(” + ezh + “)\s*\|\s*(” + ezh + “)\s*(\d{1,4})”, “g”), “$1 $3”);
s = s.replace(
new RegExp(”(” + ezh + “)\s*\|\s*(” + ezh + “)(\s+(?:” + keepAlt + “)\b.*)?”, “gi”),
“$1$3”
);
...
s = s.replace(
new RegExp(”(?:” + ecountry + “\s+)?(” + ecity + “)\s*\|\s*” + ecity + “(\s+(?:” + keepAlt + “)\b.*|\s*\d{1,4})?”, “gi”),
“$1$2”
);
s = s.replace(
new RegExp(”(?:” + ecountry + “\s+)?(” + ecity + “)\s*\|\s*” + ecountry + “(\s+(?:” + keepAlt + “)\b.*|\s*\d{1,4})?”, “gi”),
“$1$2”
);
s = s.replace(
new RegExp(ecountry + “\s+(” + ecity + “)(\s+(?:” + keepAlt + “)\b.*|\s*\d{1,4})?”, “gi”),
“$1$2”
);
}
return s;
...
- Base64 编解码（纯 JS，不依赖 TextEncoder/TextDecoder）
- ========================================================= */
