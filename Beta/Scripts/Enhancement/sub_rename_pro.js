...
;(function () {
  ...
  const twFlagMode = String(args.TWFLAG || "0");
  ...
  function parseArgument(argStr) {
    ...
    // 支持 key=value&key2=value2 格式，value 中可含空格（URL 编码或裸空格均可）
  ...
  function shouldSkipRename(name) {
    return SKIP_RENAME_PATTERNS.some(r => r.test(String(name)));
  }
  ...
  function replaceAliasSafely(text, alias, replacement) {
    ...
    const pattern = alias.trim().split(/\s+/).map(escapeRegExp).join("[\\s_\\-+]+");
  ...
  function rewriteSubscriptionLines(text, pfx, twMode) {
    return text.split(/\r?\n/).map(line => {
      ...
      if (hashIdx !== -1) {
        ...
        let nodeName = after;
        try { nodeName = decodeURIComponent(after); } catch {}
        nodeName = applySmartRewrite(nodeName, pfx, twMode);
        return before + encodeURIComponent(nodeName);
      }
  ...
  function rewriteClashYamlProxyNames(text, pfx, twMode) {
    return text.split(/\r?\n/).map(line => {
      ...
      const newName = applySmartRewrite(oldName, pfx, twMode);
      return `${head}"${newName}"`;
    }).join("\n");
  }
  ...
  /* -------------------------------------------------------
   * Base64 编解码（纯 JS，兼容 Surge JS 引擎，无 TextEncoder 依赖）
   * ------------------------------------------------------- */

  function encodeTextToBase64(text) {
    // Surge 环境有原生 btoa，但需先将 UTF-8 字符串转为 binary string
  ...
  // UTF-8 字符串 → Uint8Array（不依赖 TextEncoder）
  function utf8ToBytes(str) {
    const out = [];
    for (let i = 0; i < str.length; ) {
      let cp = str.charCodeAt(i++);
      if (cp >= 0xD800 && cp <= 0xDBFF && i < str.length) {
        const trail = str.charCodeAt(i);
  ...
  // Uint8Array → UTF-8 字符串（不依赖 TextDecoder）
