...
- 版本：2026-03-18R4
- 
- · 支持 Egern Widget（medium / large 自动适配）
- · 支持 Surge / Loon / QX 面板文字输出
- · 参数优先级：ctx.env > $argument > BoxJS
- · medium：单行最多 5 个机场
- · large：超过 5 个自动分两行，最多 10 个
- ========================================================= */
...
try { if (typeof localStorage !== “undefined”) return { read: k => localStorage.getItem(k) }; } catch (*) {}
return { read: () => null };
})();
...
if (panel) {
if (typeof panel === “string”) { try { panel = JSON.parse(panel); } catch (*) {} }
...
text: “%”,
font: { size: “caption1” },
textColor: { light: “rgba(255,255,255,0.7)”, dark: “rgba(255,255,255,0.7)” },
padding: [0, 0, 2, 0]
}
]
},
...
maxLines: 1, minScale: 0.8, textAlign: “center”
},
{
type: “text”,
text: info.error || “获取失败”,
font: { size: “caption2” },
textColor: textWhiteSoft,
maxLines: 1, textAlign: “center”
}
]
};
}
...
textColor: textWhite,
maxLines: 1, minScale: 0.7, textAlign: “center”
},
// 机场名
{
type: “text”,
...
font: { size: “caption2”, weight: “semibold” },
textColor: textWhiteSub,
maxLines: 1, minScale: 0.75, textAlign: “center”
},
// 到期日期
{
type: “stack”,
...
// 仪表盘尺寸自适应
const gaugeSize = isLarge
? (count > 5 ? 58 : 68)
: (count <= 2 ? 80 : count <= 3 ? 72 : count <= 4 ? 64 : 56);
...
contentRows = [
  {
    ...
    gap: 4,
    children: row1.map(info => buildSubColumn(info, gaugeSize))
  },
  {
    type: "stack",
    direction: "row",
    ...
    gap: 4,
    padding: [10, 0, 0, 0],
    children: row2.map(info => info
      ? buildSubColumn(info, gaugeSize)
      : { type: "stack", flex: 1 }
    )
  }
];
...
alignItems: “flex-start”,
gap: 4,
children: displayInfos.map(info => buildSubColumn(info, gaugeSize))
}];
}
...
direction: “column”,
gap: 0,
padding: isLarge ? [14, 16, 14, 16] : [12, 14, 12, 14],
children: [headerRow, …contentRows]
}]
};
}
...
if (ctx) {
// 从 ctx 读取当前 widget 尺寸，Egern 自动注入
const widgetFamily = ctx.widgetFamily ?? “medium”;
