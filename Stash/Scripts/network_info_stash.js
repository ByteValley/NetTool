...
// 模块分类 · Stash 兼容（无 $network 时兜底）
// 说明：Stash 环境没有 $network；这里提供一个“空壳”避免 ReferenceError。
// 真实 IPv6 可用性由后面的 probeLandingV6() 外部探测决定。
if (typeof $network === "undefined") {
  globalThis.$network = { wifi: {}, cellular: {}, v4: {}, v6: {} };
}
// 模块分类 · 网络栈探测
const WANT_V6 = !!CFG.IPv6;

// Stash：没有 $network 的 v6 栈信息，不能用它来判断。
// 这里先按“用户想开 IPv6 就尝试”处理；真正是否可用由 probeLandingV6() 判定（V6_READY）。
const HAS_V6 = WANT_V6;
const IPV6_EFF = WANT_V6;
