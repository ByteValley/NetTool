...
// === 补丁 ①：Promise.withResolvers 兼容填充（某些环境不存在会直接炸脚本） ===
...
const $http = (op, t = 4) => {
  ...
  const HTTPError = (e, req, res) =>
    Object.assign(new Error(e), { name: "HTTPError", request: req, response: res });
...
if (url.indexOf(path) != -1) {
  ...
  // === 补丁 ②：URLSearchParams 不可用时，改用手动解析 ===
  let devId = "";
  try {
    if (typeof URLSearchParams === "function") {
      ...
      const pairs = String(reqbody).split("&");
