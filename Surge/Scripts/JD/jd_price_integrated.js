...
const STYLE = (ARG.style||"").toLowerCase();      // table | raw | line
const LINE_ONLY = /^(1|true|yes)$/i.test(String(ARG.line_only||""));
...
function getDevId(){const body=getval(KEY_BODY); if(!body){ notify(APP,"请先打开【慢慢买】APP—我的","确保提示‘获取ck成功’"); return null; } return parseQS(body)?.c_mmbDevId || getval(KEY_DEVID); }
function extractId(u){let m=u.match(/\/product\/graphext\/(\d+)\.html/); return m&&m[1]||null;}
...
function requestHistoryPrice(id){
  ...
  return $http(reqShare).then(res=>{
    ...
    const reqTrend = { method:"post", url:"https://apapia-history-weblogic.manmanbuy.com/h5/share/trendData", headers:{ "content-type":`multipart/form-data; boundary=${boundary}` }, body };
...
function tableHtml(r){
  const arr = r?.priceRemark?.ListPriceDetail || [];
  const map = {"双11价格":"双十一价格","618价格":"六一八价格","30天最低价":"三十天最低","60天最低价":"六十天最低","180天最低价":"一百八最低"};
  let rows=""; for (const it of arr){ if (/历史最高|常购价/.test(it?.Name||"")) continue; rows += `<tr><td>${map[it.Name]||it.Name||""}</td><td>${it.Price??"-"}</td><td>${it.Date||""}</td><td>${it.Difference==='-'?"":(it.Difference||"")}</td></tr>`; }
...
if (MODE==="popup" && typeof $response === "undefined") {
  (async ()=>{
    try{
      const m = extractId(REQ_URL);
      if (!m){ done({}); return; }
      const data = await requestHistoryPrice(m);
      if (data && data.ok===1){
        const r=data.result||{};
        const title=r?.trendData?.title||("商品 "+m);
        const tip=(r?.priceRemark?.Tip||"")+"（仅供参考）";
        const lower=lowerMsg(r);
        const detail=summaryText(r);
        notify(title, `${lower} ${tip}`, detail, {"open-url": `https://item.jd.com/${m}.html`, "update-pasteboard": detail});
...
if (MODE==="render" && typeof $response !== "undefined") {
  (async ()=>{
    try{
      ...
      let content="";
      if (STYLE==="table")      content = tableHtml(r);
      else if (STYLE==="raw")   content = `<pre class="jd-price-pre">${summaryText(r).replace(/[&<>]/g,s=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[s]))}</pre>`;
      else if (STYLE==="line")  content = (LINE_ONLY?"":tableHtml(r)) + lineSvg(r);
      else                      content = tableHtml(r); // 默认表格
      ...
      try{ $done($response); }catch(__){ done({}); }
    }
  })();
}
