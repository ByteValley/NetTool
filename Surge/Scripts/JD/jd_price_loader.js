// 京东比价（带 Auto 模式与触发提示）
// MITM：in.m.jd.com, item.m.jd.com, apapia-sqk-weblogic.manmanbuy.com

const APP = "京东比价";
const SECRET = "3E41D1331F5DDAFCD0A38FE2D52FF66F";
const KEY_BODY = "manmanbuy_val";   // 保存原始 token body（含 c_mmbDevId）
const KEY_DEVID = "慢慢买CK";       // 保存 c_mmbDevId（兜底）
...
let MODE = (ARG.mode||"").toLowerCase();
const DEBUG = String(ARG.debug||"").toLowerCase()==="1" || String(ARG.debug||"").toLowerCase()==="true";
...
/* ========= A) 获取 token ========= */
if (/baoliao\/center\/menu/.test(REQ_URL) && (MODE==="token" || MODE==="")) {
  try{
    ...
    notify(APP, (MODE?`触发 ${MODE}`:"触发 token(auto)"), devId?("c_mmbDevId="+devId):body.slice(0,220));
  }catch(e){ notify(APP+"｜获取token异常","", String(e&&e.message||e)); }
  return done({});
}

/* ===== 分支自动检测（保底） ===== */
const IS_JD_PAGE = /https?:\/\/(?:in|item)\.m\.jd\.com\/(?:(?:product\/(?:[A-Za-z]+\/)?\d+\.html)|(?:ware\/view\?(?:[^#]*?)(?:wareId|sku|id)=\d+)(?:[^#]*)|(?:.+?\/\d+\.html))/.test(REQ_URL);
if (!MODE && IS_JD_PAGE) {
  MODE = (typeof $response === "undefined") ? "popup" : "render";
}

/* ========= B) 弹窗（request） ========= */
if (MODE==="popup" && typeof $response === "undefined" && IS_JD_PAGE) {
  const id = (function(u){
    let m=u.match(/\/product\/(?:[A-Za-z]+\/)?(\d+)\.html/); if(m&&m[1])return m[1];
    m=u.match(/[?&](?:wareId|sku|id)=(\d+)/); if(m&&m[1])return m[1];
    m=u.match(/\/(\d+)\.html(?:[#?]|$)/); if(m&&m[1])return m[1];
  ...
  if (DEBUG) notify(APP, "触发 popup", REQ_URL);
  if (!id) return done({});
  (async ()=>{
    try{
      ...
      if (data && data.ok === 1){
        ...
        notify(APP+"｜接口返回异常", "", JSON.stringify(data||{}).slice(0,300));
      }
    }catch(e){
...
/* ========= C) 页面渲染（response） ========= */
if (MODE==="render" && typeof $response !== "undefined" && IS_JD_PAGE) {
  const id = (function(u){
    let m=u.match(/\/product\/(?:[A-Za-z]+\/)?(\d+)\.html/); if(m&&m[1])return m[1];
    m=u.match(/[?&](?:wareId|sku|id)=(\d+)/); if(m&&m[1])return m[1];
    m=u.match(/\/(\d+)\.html(?:[#?]|$)/); if(m&&m[1])return m[1];
  ...
  if (DEBUG) notify(APP, "触发 render", REQ_URL);
