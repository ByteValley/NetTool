/***** 京东比价（四合一·单脚本） *****
 * 功能：获取 token（慢慢买）、弹窗比价（request）、页面内渲染（response：表格/原始/折线）
 * 兼容：Surge / Loon / Stash / Egern / Quantumult X
 * MITM：in.m.jd.com, apapia-sqk-weblogic.manmanbuy.com
 ************************************/

const APP = "京东比价";
const SECRET = "3E41D1331F5DDAFCD0A38FE2D52FF66F";
const KEY_BODY = "manmanbuy_val"; // 存整个慢慢买 body（含 c_mmbDevId）
const KEY_DEVID = "慢慢买CK";      // 直接存 c_mmbDevId（与上游脚本保持一致字段名）
...
function getval(k) {
  try { return isQX ? $prefs.valueForKey(k) : $persistentStore.read(k); } catch (_) { return null; }
}
function setval(k, v) {
  try { return isQX ? $prefs.setValueForKey(v, k) : $persistentStore.write(v, k); } catch (_) { return false; }
}
function http(op) {
  return new Promise((resolve, reject) => {
    if (isQX) {
      $task.fetch(typeof op === "string" ? { url: op } : op).then(res => resolve(res), err => reject(err));
    } else {
      const m = (op.method || "GET").toLowerCase();
      const cb = (err, resp, body) => {
        if (!resp) resp = {};
        resp.status = resp.status || resp.statusCode;
        resolve({ statusCode: resp.status, headers: resp.headers || {}, body });
      };
      if (m === "post") $httpClient.post(op, cb);
      else $httpClient.get(op, cb);
    }
  });
}
...
function parseQueryString(queryString) { const jsonObject = {}; const pairs = (queryString||"").split('&'); for (const p of pairs) { const kv = p.split('='); const key = decodeURIComponent(kv[0] || ''); const val = decodeURIComponent(kv[1] || ''); if (key) jsonObject[key] = val; } return jsonObject; }
...
if (MODE === "token" && /apapia-sqk-weblogic\.manmanbuy\.com\/baoliao\/center\/menu/.test(reqUrl)) {
  try {
    const body = ($request && $request.body) || "";
    setval(KEY_BODY, body);

    // 兼容 URLSearchParams 不可用：手动解析
    let devId = "";
    try {
      if (typeof URLSearchParams === "function") {
        ...
        const kvs = String(body).split("&");
        for (const it of kvs) {
          const kv = it.split("=");
          if (decodeURIComponent(kv[0] || "") === "c_mmbDevId") { devId = decodeURIComponent(kv[1] || ""); break; }
        }
      }
    } catch(_) {}
    setval(KEY_DEVID, devId);
    notify(APP, "获取 ck 成功 🎉", devId ? ("c_mmbDevId=" + devId) : body.slice(0, 160));
  } catch (e) {
    notify(APP + "｜获取token异常", "", String(e && (e.stack || e)));
  }
  return done({});
}
...
function getDevId() {
  ...
  const idSaved = getval(KEY_DEVID) || "";
  const idFromBody = parseQueryString(raw).c_mmbDevId || "";
  const devId = idSaved || idFromBody;
  if (!devId) {
    notify(APP, "请先打开【慢慢买】App → 我的", "未获取到 c_mmbDevId");
    return null;
  }
  return devId;
}
...
async function fetchHistory(id) {
  const c_mmbDevId = getDevId();
  if (!c_mmbDevId) throw new Error("缺少 c_mmbDevId");
  ...
  // ① /app/share
  const res1 = await http({ method: "POST", url: "https://apapia-history-weblogic.manmanbuy.com/app/share", headers, body: jsonToQueryString(shareBody) });
  let j1;
  try { j1 = JSON.parse(res1.body || "{}"); } catch(_) {}
  if (!j1 || j1.code !== 2000 || !j1.data) throw new Error("share 接口异常：" + (j1 && j1.msg || "无返回"));

  const urlObj = new URL(j1.data);
  const fields = {
    shareId: urlObj.searchParams.get("shareId"),
    sign: urlObj.searchParams.get("sign"),
    spbh: urlObj.searchParams.get("spbh"),
    url: urlObj.searchParams.get("url"),
  };

  // ② /h5/share/trendData（multipart）
  ...
  let j2; try { j2 = JSON.parse(res2.body || "{}"); } catch(_) {}
  if (!j2) throw new Error("trendData 解析失败");
  return j2; // 结构：{ ok: 1, result: {...} } 或 { ok:0, msg: ... }
}

/* ===== 展示工具（表格/原始/折线） ===== */
function lowerMsg(r){
  const l = r.priceRemark && r.priceRemark.lowestPrice;
  const d = r.priceRemark && r.priceRemark.lowestDate;
...
function historySummary(result) {
  ...
  const list = (result.priceRemark && result.priceRemark.ListPriceDetail) || [];
  const arr = [];
  for (const it of list) {
    const n = it.Name || "";
    if (/历史最高|常购价/.test(n)) continue;
    arr.push({ Name: n, Price: it.Price, Date: it.Date || toDate(), Difference: it.Difference });
  }
  return arr;
}
function summaryText(result) {
  const list = historySummary(result);
  const maxW = list.reduce((m,x)=>Math.max(m, String(x.Price||"-").length), 0);
  let out = "";
  ...
  for (const it of list) {
    ...
    if (price !== "-") {
      if (!price.includes(".") && price.length + 1 === maxW) price += ".";
      const padChar = price.includes(".") ? "0" : " ";
      while (price.length < maxW) price += padChar;
    }
    out += `${name}  ${price}  ${it.Date}  ${it.Difference==='-'?'':(it.Difference||'')}\n`;
  }
  return out.replace(/\n$/, "");
}
function tableHtml(result) {
  const arr = (result.priceRemark && result.priceRemark.ListPriceDetail) || [];
  const map={"双11价格":"双十一价格","618价格":"六一八价格","30天最低价":"三十天最低","60天最低价":"六十天最低","180天最低价":"一百八最低"};
  let rows="";
  for (const it of arr) {
    const n = it.Name || "";
    if (/历史最高|常购价/.test(n)) continue;
    rows += `<tr><td>${map[n]||n}</td><td>${it.Price||"-"}</td><td>${it.Date||""}</td><td>${it.Difference==='-'?"":(it.Difference||"")}</td></tr>`;
  }
...
function lineHtml(result){
  const list = result.priceTrend || [];
  const pts=[]; for (const x of list){ const val = Number(x && (x.price ?? x.Price ?? x.p)); if(!isNaN(val)) pts.push(val); }
  if (!pts.length) return '<div class="jd-price-muted">暂无折线数据</div>';
  const W=320,H=120,P=10,xmax=Math.max(1,pts.length-1);
  let ymin=Infinity,ymax=-Infinity;
  for (const p of pts){ ymin=Math.min(ymin,p); ymax=Math.max(ymax,p); }
  if (!isFinite(ymin) || !isFinite(ymax)) { ymin=0; ymax=1; }
  const sx=(W-2*P)/xmax, sy=(H-2*P)/Math.max(1,(ymax-ymin)||1);
  let points="";
  for (let i=0;i<pts.length;i++){ const xx=P+i*sx; const yy=H-P-((pts[i]-ymin)*sy); points+=(i?",":"")+xx+","+yy; }
  ...
  (async () => {
    try {
      ...
      if (data && data.ok === 1) {
        ...
        const msg = lowerMsg(r) + tip;
        const detail = summaryText(r);
        notify(title, msg, detail, {"open-url": `https://item.jd.com/${id}.html`, "update-pasteboard": detail});
      } else if (data && data.ok === 0 && data.msg) {
        notify("比价结果", "", "慢慢买提示您：" + data.msg);
      }
      done({});
    } catch (e) { notify(APP+"｜接口异常","",String(e)); done({}); }
  })();
  // request 分支必须 return
  return;
}
  ...
  (async () => {
    try {
      ...
      if (style_table) {
        ...
        content = line_only ? "" : tableHtml(r);
        content += lineHtml(r);
      } else if (style_raw) {
        const txt = summaryText(r).replace(/[&<>]/g, s=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[s]));
        content = `<pre class="jd-price-pre">${txt}</pre>`;
      } else {
        // 默认表格
        content = tableHtml(r);
      }
      ...
      $done({ body: html });
    } catch (_) { try{$done($response);}catch(_e){done({});} }
  })();
  return;
}
