/**
 * 节日倒数·永久版（两行合并）
 * 动态计算：阳历固定/规则日、农历节日、二十四节气；正日 06:00 后单次祝词通知
 * 标题池修正：19=农历信息；20=阳历信息
 * 作者：ByteEden & ChatGPT  |  版本：2025-11-06
 */
(() => {
/* ========== 工具 ==========\ */
const tnow = new Date();
...
function titleRandom(daysToNext, nextName) {
  const dic = {
    1: "距离放假，还要摸鱼多少天？🥱",
    2: "坚持住，就快放假啦！💪",
    3: "上班好累呀，好想放假😮‍💨",
    ...
    16: "今天的目标是先活下去",
    17: "给自己加个鸡腿！",
    18: nextName ? `下一个：${nextName}` : "用力生活，用力摸鱼",
    19: titleLunar,
    20: titleSolar
  };
...
function lunarSet(year) {
  ...
  const converted = base.map(([n,[lm,ld]]) => [n, calendar.lunar2solar(year, lm, ld).date]);
  converted.push(["除夕", lunarNewYearEveSolar(year)]);
  return converted;
}
function buildYear(year) {
  const set = [...gregorianSet(year), ...solarTerms(year), ...lunarSet(year)];
...
/* ========== 列表与最近三项 ========== */
const yearA = [...buildYear(y), ...buildYear(nextY)];
function buildB(listA) {
  const res = [];
  for (const [name, date] of listA) if (bless[name]) res.push([name, date, bless[name]]);
  return res;
}
const yearB = buildB(yearA);
...
const A3 = nextTrip(yearA), B3 = nextTrip(yearB);
const dA0 = dateDiff(todayStr, A3[0][1]), dA1 = dateDiff(todayStr, A3[1][1]), dA2 = dateDiff(todayStr, A3[2][1]);
const dB0 = dateDiff(todayStr, B3[0][1]), dB1 = dateDiff(todayStr, B3[1][1]), dB2 = dateDiff(todayStr, B3[2][1]);

/* ========== 正日提醒（B 源，06:00 后每日一次） ========== */
if (dB0 === 0 && tnow.getHours() >= 6) {
  const key = "timecardpushed_b";
  if ($persistentStore.read(key) !== B3[0][1]) {
    $persistentStore.write(B3[0][1], key);
    $notification.post(`🎉今天是 ${B3[0][1]} ${B3[0][0]}`, "", B3[0][2] || "节日快乐！");
  }
}
...
/* ========== 两行面板输出 ========== */
const lineA = (dA0 === 0)
  ? `今天：${A3[0][0]} | ${A3[1][0]}${dA1}天 | ${A3[2][0]}${dA2}天`
  : `${A3[0][0]}${dA0}天 | ${A3[1][0]}${dA1}天 | ${A3[2][0]}${dA2}天`;
const lineB = `${B3[0][0]}:${dB0===0?"🎉":"还剩"+dB0+"天"}, ${B3[1][0]}:${dB1}天, ${B3[2][0]}:${dB2}天`;

$done({
  title: titleRandom(dA0, A3[0][0]),
  icon: iconNow(dA0),
  "icon-color": iconColor(dA0),
  content: `${lineA}\n\n${lineB}`
});
})();
