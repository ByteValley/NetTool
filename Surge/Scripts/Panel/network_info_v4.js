...
;(async () => {
  // —— 直连（并发）
  const pCn4 = getDirectV4(DOMESTIC_IPv4).catch(()=>({}));
  const pCn6 = IPv6_ON ? getDirectV6(DOMESTIC_IPv6).catch(()=>({})) : Promise.resolve({});

  // —— 先触发“落地”请求（它会在 Surge 日志里留下可用于解析入口的记录）
  const pPx4 = getLandingV4(LANDING_IPv4).catch(()=>({}));
  const pPx6 = IPv6_ON ? getLandingV6(LANDING_IPv6).catch(()=>({})) : Promise.resolve({});

  // —— 读取入口：带重试（等待前面的落地请求写入最近请求日志）
  const { policyName, entranceIP } = await getPolicyAndEntranceRetry(4, 200).catch(()=>({}));

  // —— 入口位置：国内/国际双源
  const ent = isIP(entranceIP||'') ? await getEntranceBundle(entranceIP).catch(()=>({ ip: entranceIP })) : {};

  // —— 等待直连/落地结果
  const [cn, cn6, px, px6] = await Promise.all([pCn4, pCn6, pPx4, pPx6]);
  ...
  const directLines = [];
  if (policyName) directLines.push(`代理策略: ${policyName}`);   // 先显示“代理策略”
...
function fmtISP(isp, locStr){
  ...
  if (/^(电信|China Telecom.*|Chinanet|CTGNET|CN2.*)$/i.test(s))    return '中国电信';
...
async function getPolicyAndEntranceOnce(){
  ...
  const hit  = reqs.slice(0, 100).find(i => ENT_SOURCES_RE.test(i.URL || ''));
  if (!hit) return {};
  const ip = extractIP(hit.remoteAddress || '');
  return { policyName: hit.policyName || '', entranceIP: ip || '' };
}

async function getPolicyAndEntranceRetry(times=3, baseDelay=200){
  for (let i=0; i<times; i++){
    const r = await getPolicyAndEntranceOnce().catch(()=>({}));
    if (r?.entranceIP || r?.policyName) return r;
    if (i < times - 1) await sleep(baseDelay * Math.pow(1.6, i)); // 指数退避
  }
  return {};
}
