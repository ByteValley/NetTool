...
var $prefs = {
  removeValueForKey: key => {
    ...
    console.log(err)
    $notification.post('Script Hub: 脚本转换', '❌ streaming-ui-check.js.txt', err)
    return result
  },
  valueForKey: key => {
...
$configuration.sendMessage(message).then(resolve => {
    ...
    if (resolve.ret) {
      ...
      console.log(output);
      _scriptSonverterDone({"title":result["title"],"htmlMessage":content})
...
.finally(() => {
  ...
  $configuration.sendMessage(message).then(resolve => {
    ...
    _scriptSonverterDone({"title":result["title"],"htmlMessage":`<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">`+'----------------------</br></br>'+"🚥 检测异常"+'</br></br>----------------------</br>'+ output + `</p>`})
}
  );
