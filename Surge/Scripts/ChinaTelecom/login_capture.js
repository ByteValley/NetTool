...
[Script]
电信登录地址抓取 = type=http-request,pattern=^https:\/\/e\.dlife\.cn\/user\/loginMiddle\.do(?:\?.*)?$,requires-body=0,script-path=https://your.cdn/login_capture.js,script-update-interval=0
电信入口地址抓取 = type=http-request,pattern=^https:\/\/e\.dlife\.cn\/index\.do(?:\?.*)?$,requires-body=0,script-path=https://your.cdn/login_capture.js,script-update-interval=0
*/
...
function capture() {
  try {
    ...
    const urlDec = safeDecode(raw);

    // 仅处理我们关心的入口
    if (!/\/user\/loginMiddle\.do\b|\/index\.do\b/i.test(urlDec)) {
      $.done();
      return;
    }

    // 1) 优先 query 里的 loginUrl=...
    const mLogin = urlDec.match(/[?&]loginUrl=([^&]+)/i);
    let loginUrl = mLogin ? safeDecode(mLogin[1]) : urlDec;

    // 2) 去掉签名（一次性/时效性强，保留会造成频繁变化或失效）
    loginUrl = loginUrl.replace(/&sign=[^&]*/i, "");

    // 3) 若仍不是 http(s) 开头，兜底抽全文首个链接
    ...
    // 4) 校验 + 仅变更时写入（降噪）
    if (!/^https?:\/\//i.test(loginUrl)) {
      $.notify("中国电信", "登录地址获取失败", "未在请求中找到有效登录地址");
      $.error({ raw, urlDec });
    } else {
      const key = "china_telecom_loginUrl";
      const old = $.read(key) || "";
      if (old !== loginUrl) {
        $.write(loginUrl, key);
        $.write(urlDec,   key + "_raw");
        $.notify("中国电信", "登录地址获取成功", loginUrl);
        $.info(`登录地址：${loginUrl}`);
      } else {
        $.log("[CT-capture] 登录地址未变化，跳过写入/通知");
      }
    }
  } catch (e) {
    $.error(e);
    $.notify("中国电信", "登录地址获取失败", String(e));
  } finally {
    $.done();
  }
}
