...
if ($request) captureLoginUrl();

function captureLoginUrl() {
  try {
    const url = $request?.url || "";
    // 仅处理目标地址
    if (!/^https:\/\/e\.dlife\.cn\/user\/loginMiddle/i.test(url)) return $.done();

    // 不再裁剪 &sign —— 原样保存整条 SSO 登录链接
    const loginUrl = url;

    // 基本校验：提示是否包含 sign
    const hasSign = /[?&]sign=/.test(loginUrl);

    // 持久化
    $.write(loginUrl, "china_telecom_loginUrl");

    // 通知与日志
    const sub = hasSign ? "登录地址获取成功（含 sign）" : "登录地址获取成功（未检测到 sign，可能不可用）";
    $.notify("中国电信", sub, loginUrl);
    $.info(`登录地址：${loginUrl}`);
  } catch (e) {
