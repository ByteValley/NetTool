...
;(async () => {
  ...
  try {
    ...
    // 兼容 Surge 模块 arguments（k:v,k:v）和 querystring（a=1&b=2）
    function parseArgs() {
      if (typeof $argument === "undefined" || !$argument) return {};
      const raw = String($argument).trim();
      const out = {};

      // 尝试按 querystring 解析（Egern / 手动 argument= 形式）
      if (raw.includes("=")) {
    ...
    const calendar = {
      ...
      solar2lunar: function (Y,M,D){
        ...
        let isTodayObj=new Date(),isToday=(isTodayObj.getFullYear()===y&&isTodayObj.getMonth()+1===m&&isTodayObj.getDate()===d);
        let nWeek=obj.getDay(),cWeek=this.nStr1[nWeek];if(nWeek===0)nWeek=7;
        const year=i;let leap=this.leapMonth(i),isLeap=false;
        ...
        if(offset<0){offset+=temp;--i}
        const month=i;const day=offset+1;const sm=m-1;
        ...
        const fest=this.festival;const lfest=this.lFestival;
        const festKey=m+"-"+d;let lfestKey=month+"-"+day;
        if(month===12&&day===29&&this.monthDays(year,month)===29){lfestKey="12-30"}
        return{
          date:solarDate,
          lunarDate:lunarDate,
      ...
      lunar2solar: function (y,m,d,isLeap){
        y=parseInt(y);m=parseInt(m);d=parseInt(d);isLeap=!!isLeap;
        ...
        for(let i=1;i<m;i++){
          ...
          if(!isAdd){
            if(leap<=i&&leap>0){offset+=this.leapDays(y);isAdd=true}
          }
    ...
    const lunarNow = calendar.solar2lunar(
      tnow.getFullYear(),
      tnow.getMonth() + 1,
      tnow.getDate()
    );
