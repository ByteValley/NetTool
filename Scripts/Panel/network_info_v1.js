 ...
 *  · SD_CONCURRENCY         服务检测并发数                  默认 3（建议 2~4）
...
const CFG = {
  ...
  /* —— 服务检测并发 —— */
  SD_CONCURRENCY: toNum(ENV("SD_CONCURRENCY", 3), 3),
...
log("info", "Start", JSON.stringify({
  ...
  SD_CONCURRENCY: CFG.SD_CONCURRENCY
}));
...
;(async () => {
  ...
  const [cn, cn6] = await Promise.all([
    ...
    IPV6_EFF ? getDirectV6(DOMESTIC_IPv6).catch((e) => { log("warn", "DirectV6", String(e)); return {}; }) : Promise.resolve({})
  ]);
  ...
  const ent4 = isIP(entrance4 || "")
    ? await getEntranceBundle(entrance4).catch((e) => { log("warn", "EntranceBundle v4", String(e)); return { ip: entrance4 }; })
    : {};
  const ent6 = isIP(entrance6 || "")
    ? await getEntranceBundle(entrance6).catch((e) => { log("warn", "EntranceBundle v6", String(e)); return { ip: entrance6 }; })
    : {};

  const t2 = Date.now();
  const [px, px6] = await Promise.all([
    getLandingV4(LANDING_IPv4).catch((e) => { log("warn", "LandingV4", String(e)); return {}; }),
    IPV6_EFF ? getLandingV6(LANDING_IPv6).catch((e) => { log("warn", "LandingV6", String(e)); return {}; }) : Promise.resolve({})
  ]);
...
function httpGet(url, headers = {}, timeoutMs = null, followRedirect = false) {
  return new Promise((resolve, reject) => {
    ...
    const DEF_TO = Math.max(2000, (Number(CFG.Timeout) || 12) * 1000);
    req.timeout = (timeoutMs != null) ? timeoutMs : DEF_TO;
...
// ====================== 统一抓取器（减少重复，增强 v4/v6 校验） ======================
async function trySources(order, sourceMap, { preferLogTag, needCityPrefer = false, family = null }) {
  ...
  for (const key of order) {
    ...
    try {
      ...
      if (family === "v4" && res.ip && !isIPv4(res.ip)) {
        log("warn", `${preferLogTag} family mismatch(v4 expected)`, key, _maskMaybe(res.ip));
        continue;
      }
      if (family === "v6" && res.ip && !isIPv6(res.ip)) {
        log("warn", `${preferLogTag} family mismatch(v6 expected)`, key, _maskMaybe(res.ip));
        continue;
      }
...
async function getDirectV4(preferKey) {
  ...
  if (!res || !res.ip) {
    try {
      ...
      if (out.ip && isIPv4(out.ip)) return out;
      return {};
    } catch (e2) {
...
async function loc_chain(ip) {
  try { return await withRetry(() => ENT_LOC_CHAIN.ipapi(ip), 1); } catch {}
  try { return await withRetry(() => ENT_LOC_CHAIN.ipwhois(ip), 1); } catch {}
...
async function getEntranceBundle(ip) {
  ...
  if (ENT_CACHE.ip === ip && ENT_CACHE.data) {
    log("info", "Entrance cache EXPIRED", { ip: _maskMaybe(ip), age_ms: (nowT - ENT_CACHE.t), ttl_ms: ENT_TTL_SEC * 1000 });
  } else {
...
// ====================== 服务清单解析 & 检测（限并发 + 重试） ======================
async function runWithLimit(fns, limit = 3) {
  const ret = new Array(fns.length);
  let idx = 0;
  ...
  return new Promise((resolve) => {
    const once = (tryNo) => {
      ...
      $httpClient.get({
        ...
        if (err || !resp) {
          log("warn", "sd_httpGet FAIL", `#${tryNo}`, url, "cost", cost + "ms", String(err || ""));
          if (tryNo === 1) return setTimeout(() => once(2), 120);
  ...
  return new Promise((resolve) => {
    const once = (tryNo) => {
      ...
      $httpClient.post({
        ...
        const cost = sd_now() - start;
        if (err || !resp) {
          log("warn", "sd_httpPost FAIL", `#${tryNo}`, url, "cost", cost + "ms", String(err || ""));
          if (tryNo === 1) return setTimeout(() => once(2), 120);
          return resolve({ ok: false, status: 0, cost, headers: {}, data: "" });
        }
        const status = resp.status || resp.statusCode || 0;
...
async function runServiceChecks() {
  try {
    ...
    const conc = Math.max(1, Math.min(6, Number(CFG.SD_CONCURRENCY) || 3));
    const fns = order.map((k) => () => SD_TESTS_MAP[k] && SD_TESTS_MAP[k]());
    const lines = await runWithLimit(fns, conc);
...
function zhHansToHantOnce(s) {
  ...
  const charMap = {
    ...
    "时": "時", "区": "區", "术": "術", "产": "產", "广": "廣", "电": "電",
    "联": "聯", "动": "動", "数": "數", "汉": "漢", "气": "氣", "历": "曆", "宁": "寧"
  };
