 ...
 *  · 直连 IPv6：ipify | ident | ipsb | ddnspod | neu6 | icanhaz | ifconfig（并发取首个成功）
 *  · 落地 IPv4：ipsb | ipapi | ipwhois（失败逐级回退）
 *  · 落地 IPv6：ipify | ident | ipsb | icanhaz | ifconfig（并发取首个成功）
 *
 * 入口 · 策略名获取（稳态）
 ...
 *  · SD_TIMEOUT             单项检测超时（秒）             默认=Timeout，最小 2，0=跟随 Timeout
 *  · IPv6                   启用 IPv6                      默认 1
 *  · MASK_IP                脱敏 IP                        默认 1
 *  · MASK_POS               脱敏位置                       默认 1（未设时随 MASK_IP）
 *  · DOMESTIC_IPv4          直连 IPv4 源                   默认 ipip
 *  · DOMESTIC_IPv6          直连 IPv6 源                   默认 ipify
 *  · LANDING_IPv4           落地 IPv4 源                   默认 ipsb
 *  · LANDING_IPv6           落地 IPv6 源                   默认 ipify
 * ========================================================= */
...
const CONSTS = Object.freeze({
    ...
    // All timeouts are in seconds (Surge/Loon $httpClient.timeout uses seconds)
    PRETOUCH_TO_SEC: 1,

    // Retry/backoff uses milliseconds (setTimeout/sleep)
    RETRY_DELAY_MS: 260,

    // Minimum per-request timeout (seconds)
    SD_MIN_TIMEOUT_SEC: 2,

    LOG_RING_MAX: 120,
    DEBUG_TAIL_LINES: 18,

    // Entrance GeoIP request timeout floor (seconds)
    ENT_MIN_REQ_TO_SEC: 3,
...
const CFG = {
    ...
    DOMESTIC_IPv6: (() => {
        // new default: ipify (more stable v6)
        const v = ENV('DOMESTIC_IPv6', 'ipify');
        if (v !== '' && v != null) return v;
        return $args.DOMIC_IPv6 || 'ipify';
    })(),
    LANDING_IPv4: ENV('LANDING_IPv4', 'ipsb'),
    LANDING_IPv6: ENV('LANDING_IPv6', 'ipify'),
    ...
    /* SD_TIMEOUT: seconds; 0 or empty = follow Timeout */
...
// SD_TIMEOUT: seconds; 0/empty = follow Timeout; minimum = SD_MIN_TIMEOUT_SEC
const SD_TIMEOUT_SEC = (() => {
    const raw = Number(CFG.SD_TIMEOUT);
    const fallback = Number(CFG.Timeout) || 8;
    if (!Number.isFinite(raw) || raw <= 0) return Math.max(CONSTS.SD_MIN_TIMEOUT_SEC, fallback);
    return Math.max(CONSTS.SD_MIN_TIMEOUT_SEC, raw);
})();

// IPv6 requests are kept shorter to avoid dragging whole panel
const V6_TO_SEC = Math.min(SD_TIMEOUT_SEC, 3);
...
const IP_HEADERS = Object.freeze({
    'User-Agent': 'Mozilla/5.0 (compatible; NetworkInfo/1.0; +https://example.invalid)'
});

// —— 默认尝试顺序（集中管理）——
const ORDER = Object.freeze({
    directV4: ['cip', 'ipip', 'pingan', '163', '126', 'bilibili'],
    landingV4: ['ipsb', 'ipapi', 'ipwhois'],

    directV6: ['ipify', 'ident', 'ipsb', 'ddnspod', 'neu6', 'icanhaz', 'ifconfig'],
    landingV6: ['ipify', 'ident', 'ipsb', 'icanhaz', 'ifconfig']
});
...
function httpGet(url, headers = {}, timeoutSec = null, followRedirect = false) {
    return new Promise((resolve, reject) => {
        ...
        const to = timeoutSec == null ? (Number(CFG.Timeout) || 8) : Number(timeoutSec);
        if (Number.isFinite(to) && to > 0) {
            req.timeout = to; // seconds
        }

        if (followRedirect) {
            req['auto-redirect'] = true;
...
// ====================== 统一抓取器（减少重复） ======================
async function trySources(order, sourceMap, {preferLogTag, needCityPrefer = false, family = ''}) {
    log('info', `${preferLogTag} begin`, JSON.stringify(order));
    let firstOK = null;

    const isFamilyOK = (ip) => {
        if (!ip) return false;
        if (family === 'v4') return isIPv4(ip);
        if (family === 'v6') return isIPv6(ip);
        return true;
    };
    ...
    if (firstOK) {
        log('info', `${preferLogTag} fallback to firstOK`, JSON.stringify({
...
function _firstValid(promises, validate) {
...
function _pickIpFromBody(body) {
    const s = String(body || '').trim();
    const m = s.match(/([0-9a-fA-F:]{2,})/);
    return m ? String(m[1]).trim() : '';
}

async function tryIPv6Ip(order) {
    const keys = order.filter((k) => IPV6_IP_ENDPOINTS[k]);
    const tasks = keys.map((key) => (async () => {
        const url = IPV6_IP_ENDPOINTS[key];
        try {
            const r = await httpGet(url, IP_HEADERS, V6_TO_SEC, true);
            const ip = _pickIpFromBody(r.body);
            if (!isIPv6(ip)) return '';
            return ip;
        } catch (e) {
    ...
    const ip = await _firstValid(tasks, (x) => isIPv6(x));
    return ip ? {ip} : {};
}
...
async function getDirectV4(preferKey) {
    ...
    const res = await trySources(order, DIRECT_V4_SOURCES, {
        preferLogTag: 'DirectV4', needCityPrefer: true, family: 'v4'
    });
    if (!res || !res.ip) {
        try {
            log('warn', 'DirectV4 all failed, final ipip fallback');
            const r = await httpGet(DIRECT_V4_SOURCES.ipip.url);
            const out = DIRECT_V4_SOURCES.ipip.parse(r) || {};
            return (out.ip && isIPv4(out.ip)) ? out : {};
        } catch (e2) {
...
async function getLandingV4(preferKey) {
    ...
    const res = await trySources(order, LANDING_V4_SOURCES, {
        preferLogTag: 'LandingV4', needCityPrefer: false, family: 'v4'
    });
...
// —— 入口位置缓存（跟 Update 联动） ——
const ENT_REQ_TO_SEC = Math.max(
    CONSTS.ENT_MIN_REQ_TO_SEC,
    Math.min(Number(SD_TIMEOUT_SEC) || 6, 8)
);
...
const ENT_LOC_CHAIN = Object.freeze({
    pingan: async (ip) => {
        const r = await httpGet('https://rmb.pingan.com.cn/itam/mas/linden/ip/request?ip=' + encodeURIComponent(ip), {}, ENT_REQ_TO_SEC);
        const d = safeJSON(r.body, {})?.data || {};
    ...
    ipapi: async (ip) => {
        const r = await httpGet(`http://ip-api.com/json/${encodeURIComponent(ip)}?lang=zh-CN`, {}, ENT_REQ_TO_SEC);
        const j = safeJSON(r.body, {});
    ...
    ipwhois: async (ip) => {
        const r = await httpGet(`https://ipwhois.app/json/${encodeURIComponent(ip)}?lang=zh-CN`, {}, ENT_REQ_TO_SEC);
        const j = safeJSON(r.body, {});
    ...
    ipsb: async (ip) => {
        const r = await httpGet(`https://api.ip.sb/geoip/${encodeURIComponent(ip)}`, {}, ENT_REQ_TO_SEC);
        const j = safeJSON(r.body, {});
...
function sd_httpGet(url, headers = {}, followRedirect = true) {
    return new Promise((resolve) => {
        ...
        $httpClient.get({
            ...
            timeout: SD_TIMEOUT_SEC,
            'auto-redirect': !!followRedirect,
            followRedirect
        }, (err, resp, data) => {
...
async function sd_testDisney() {
    ...
    const timeoutSec = (sec, code) => new Promise((_, rej) => setTimeout(() => rej(code), Math.max(0, sec) * 1000));
    const disneyGuardSec = Math.max(7, Math.min(SD_TIMEOUT_SEC, 12));

    try {
        const h = await Promise.race([home(), timeoutSec(disneyGuardSec, 'TO')]);
        const b = await Promise.race([bam(), timeoutSec(disneyGuardSec, 'TO')]).catch(() => ({}));
