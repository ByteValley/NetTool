 ...
 *  · 入口信息：若当前环境无 $httpAPI，将显示“入口不可用”提示（避免 Egern/Loon 等环境悄悄不显示入口分组）
 *  · Landing.ipwhois 源修正为 JSON 接口（避免 widget.php 非 JSON 导致回退）
 * ========================================================= */
 ...
 const SD_STR = {
     "zh-Hans": {
         ...
         entNotSupported: "当前环境不支持 $httpAPI，无法获取入口信息"
     },
     "zh-Hant": {
         panelTitle: "網路資訊 𝕏",
         wifi: "Wi-Fi",
         ...
         entNotSupported: "當前環境不支援 $httpAPI，無法取得入口資訊"
     }
 };
 ...
 /**
  * ENV：统一参数优先级（BoxJS > arguments > 代码默认）
  * 兼容 Surge 模块变量将默认值写入 $argument 的情况：
  *  - 若 argument 的值等于 defVal，则视为“未指定”，允许 BoxJS 覆盖
  */
 function ENV(key, defVal, opt = {}) {
     ...
     const argIsDefault = hasArg && canon(argRaw) === canon(defVal);

     if (hasBox) return convert(boxRaw);
     if (hasArg && !argIsDefault) return convert(argRaw);
     if (!hasBox && hasArg) return convert(argRaw);
     return defVal;
 }
 ...
 function httpGet(url, headers = {}, timeoutMs = null, followRedirect = true) {
 ...
 const HAS_HTTP_API = (typeof $httpAPI === 'function');
 let _HTTPAPI_WARNED = false;
 ...
 function httpAPI(path = '/v1/requests/recent') {
     return new Promise((res) => {
         if (HAS_HTTP_API) {
             ...
             if (!_HTTPAPI_WARNED) {
                 _HTTPAPI_WARNED = true;
                 log('warn', 'httpAPI not available (no $httpAPI)');
             }
             res(null);
         }
     });
 ...
 async function httpAPIAny(paths) {
     for (const p of paths) {
         const d = await httpAPI(p);
         if (d && Array.isArray(d?.requests)) return d;
     }
     return null;
 }
 ...
 const LANDING_V4_SOURCES = Object.freeze({
     ...
     ipwhois: {
         ...
         parse: (r) => {
             const j = safeJSON(r.body, {});
             if (j.success === false) return {ip: '', loc: '', isp: ''};
             return {
                 ip: j.ip || '',
 ...
 async function getPolicyAndEntranceBoth() {
     const data = await httpAPIAny(['/v1/requests/recent', '/requests/recent']);
     if (!data) return {policyName: '', entrance4: '', entrance6: ''};
     ...
     if (!policy && !ip4 && !ip6) {
         const d = await httpAPIAny(['/v1/requests/recent', '/requests/recent']);
 ...
 function selectServices() {
     // 优先级：BoxJS 勾选 > BoxJS 文本 > arguments > 默认全量
 ...
 ;(async () => {
     ...
     // 入口
     const hasAnyEnt = (ent4 && (ent4.ip || ent4.loc1 || ent4.loc2 || ent4.isp1 || ent4.isp2)) || (ent6 && ent6.ip);
     if (hasAnyEnt) {
     ...
     } else if (!HAS_HTTP_API) {
         pushGroupTitle(parts, '入口');
         parts.push(`(${t('entNotSupported')})`);
     }
 ...
 function zhHansToHantOnce(s) {
     ...
     const phraseMap = [
         ...
         ['当前环境不支持', '當前環境不支援'], ['无法获取入口信息', '無法取得入口資訊']
     ];
