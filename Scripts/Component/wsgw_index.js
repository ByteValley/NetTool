...
function readRoot() {
  ...
  return ensureRoot(safeParse(raw, {}));
}

function writeRoot(root) {
  return store.set(ROOT_KEY, JSON.stringify(ensureRoot(root)));
}

function getSetting(key, defVal = "") {
  const root = readRoot();
  const val = root.SGCC.Settings[key];
  return val === undefined || val === null || val === "" ? defVal : val;
}

function getCache(key, defVal = null) {
  const root = readRoot();
  const val = root.SGCC.Caches[key];
  return val === undefined || val === null || val === "" ? defVal : val;
}
...
/* =======================
 * 绑定 bizrt（统一从 SGCC.Caches.bizrt 读取）
 * ======================= */

Global.bizrt = safeParse(getCache("bizrt", "{}"), {});
let bizrt = Global.bizrt;

/* =======================
 * Logger / 账号密码（统一从 SGCC.Settings 读取）
 * ======================= */

const log = new Logger(
    SCRIPTNAME,
    isTrue(isNode() ? process.env.WSGW_LOG_DEBUG : getSetting("logDebug", false))
      ? "debug"
      : "info"
  ),
  USERNAME =
    (isNode() ? process.env.WSGW_USERNAME : getSetting("phoneNum", "")) || "",
  PASSWORD =
    (isNode() ? process.env.WSGW_PASSWORD : getSetting("password", "")) || "";
...
async function login(e, o) {
  ...
  try {
    ...
    // ✅ 新结构：写入 ComponentService.SGCC.Caches.bizrt
...
(async () => {
  ...
  .catch((e) => {
    if (/无效|失效|过期|重新获取|请求异常/.test(e)) {
      // ✅ 新结构：清理 ComponentService.SGCC.Caches.bizrt
      clearCache("bizrt");
      console.log("✅ 清理缓存数据成功");
    }
    log.error(e);
  })
  .finally(done);