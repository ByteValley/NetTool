 ...
 * @description 使用官方 App 抓取到的 token/acctoken/userId，聚合电费/电量/阶梯等数据
 *
...
class Logger {
  constructor(prefix, debug = false) { this.prefix = prefix; this.debugEnabled = !!debug }
...
async function getDayElecQuantity(index, days = 6) {
  ...
  return request95598({
    ...
    headers: { ...requestKey, token: TOKEN, acctoken: ACCTOKEN },
    data: {
      params1: { serviceCode: "", source: CFG.source, target: CFG.target, uscInfo: CFG.uscInfo, quInfo: { userId: USERID }, token: TOKEN },
      params3: {
        data: {
          acctId: USERID,
          consNo: o.consNo_dst,
